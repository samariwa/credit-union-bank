...
const TransactionDetail = () => {
  ...
  const [editToAccountDisabled, setEditToAccountDisabled] = useState(false);
  ...
  useEffect(() => {
    if (transaction?.transaction_type === "payment") {
      setEditToAccountDisabled(true); // Disable 'to account' field for payments
    } else {
      setEditToAccountDisabled(false);
    }
  ...
  const handleEditSubmit = async () => {
    if (
      ...
      transaction.to_account !== editToAccount
    ) {
      try {
        const token = localStorage.getItem("access_token");

        // Debit the initial 'to account'
        await fetch(
          `http://127.0.0.1:8000/api/accounts/${transaction.to_account}/debit/`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        // Credit the new 'to account'
        await fetch(
          `http://127.0.0.1:8000/api/accounts/${editToAccount}/credit/`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    // Proceed with the existing edit logic
    try {
      const token = localStorage.getItem("access_token");
      const response = await fetch(
        `http://127.0.0.1:8000/api/transactions/${id}/`,
      ...
      if (!response.ok) {
        ...
        if (errorData.from_account) {
          setErrorMessage(
            `The From Account ID '${editFromAccount}' does not exist. Please provide a valid account ID.`
          );
        } else if (errorData.to_account) {
          setErrorMessage(
            `The To Account ID '${editToAccount}' does not exist. Please provide a valid account ID.`
          );
        } else if (errorData.business) {
          setErrorMessage(
            `The Business ID '${editBusiness}' does not exist. Please provide a valid business ID.`
          );
        } else {
  ...
  // Adjust the edit button visibility based on transaction type
  if (
              ...
              <div className="bg-white rounded-xl p-6 max-w-md w-full">
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    className="w-full border px-3 py-2 rounded"
                    disabled={editToAccountDisabled}
                  />
                </div>
                <div className="mb-4">
                  <label className="block text-gray-700 font-medium mb-2">
